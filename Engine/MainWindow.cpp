...
MainWindow::MainWindow( HINSTANCE hInst,wchar_t * pArgs,int sw,int sh )
    :
    args( pArgs ),
    hInst( hInst ),
    sw(sw),
    sh(sh)
{
    ...
    RECT wr;
    wr.left = (desktop.right - sw) / 2;
    wr.right = sw + wr.left;
    wr.top = (desktop.bottom - sh) / 2;
    wr.bottom = sh + wr.top;
    AdjustWindowRect( &wr,WS_CAPTION | WS_MINIMIZEBOX | WS_SYSMENU,FALSE );
    hWnd = CreateWindow( wndClassName,L"Dungeon Explorer",
...
LRESULT MainWindow::HandleMsg( HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam )
{
    ...
    case WM_MOUSEMOVE:
    {
        ...
        else
        {
            ...
            {
                x = std::max( 0,x );
                x = std::min( int( sw ) - 1,x );
                y = std::max( 0,y );
                y = std::min( int( sh ) - 1,y );
                mouse.OnMouseMove( x,y );
            }
            else
            {
